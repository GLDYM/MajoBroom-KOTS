package dev.dl909.majobroom.entity;

import dev.dl909.majobroom.client.input.KeyBindings;
import dev.dl909.majobroom.common.PerspectiveMode;
...
import dev.dl909.majobroom.network.packet.BroomInputPayload;
...
public class BroomEntity extends Entity implements GeoEntity {
    ...
    private void updateMotion() {
        ...
        if (this.inputUp) {
            forwardSpeed += (float) ServerConfig.acceleration;
        }
        if (this.inputDown) {
            forwardSpeed -= (float) ServerConfig.acceleration * 0.5F;  // 后退较慢
        }
        
        // 3. 垂直速度
        double verticalSpeed = motion.y;
        if (this.inputForward) {
            verticalSpeed = Math.min(verticalSpeed + ServerConfig.verticalAcceleration,
                                    ServerConfig.maxVerticalSpeed);
        } else if (this.inputBack) {
            verticalSpeed = Math.max(verticalSpeed - ServerConfig.verticalAcceleration,
                                    -ServerConfig.maxVerticalSpeed);
        }
        ...
        // 5. 合成并应用动量衰减
        double momentum = ServerConfig.momentum;
        motion = new Vec3(
    ...
    private double getEffectiveMaxSpeed() {
        Entity passenger = getFirstPassenger();
        double baseMaxSpeed = ServerConfig.maxSpeed;
        ...
        // 如果没有穿戴任何魔女装备，应用速度惩罚
        if (!wearingMajoArmor) {
            baseMaxSpeed *= ServerConfig.noArmorSpeedPenalty;
        }
    ...
    public void loadConfigFromItemStack(ItemStack stack) {
        ...
        if (tag.contains("PerspectiveMode")) {
            setPerspectiveMode(PerspectiveMode.fromOrdinal(tag.getInt("PerspectiveMode")));
        }
    }
