 ...
 *  - GeckoLib?
 */
public class MajoClothItem extends Item implements GeoItem {
        ...
        consumer.accept(new GeoRenderProvider() {
            ...
            public GeoArmorRenderer<?, ?> getGeoArmorRenderer(ItemStack itemStack, EquipmentSlot equipmentSlot) {
                if (this.renderer == null)
                    this.renderer = new MajoClothRenderer(MajoClothItem.this);
    ...
    public void inventoryTick(ItemStack itemstack, ServerLevel level, Entity entity, EquipmentSlot slot) {
        ...
        if (entity instanceof LivingEntity livingEntity && slot == EquipmentSlot.CHEST && ServerConfig.armorBless) {
            if (level.getGameTime() % EFFECT_CHECK_INTERVAL_TICKS == 0) {
                refreshEffectIfNeeded(livingEntity, MobEffects.RESISTANCE, 3);
                refreshEffectIfNeeded(livingEntity, MobEffects.FIRE_RESISTANCE, 0);
            }
        }
